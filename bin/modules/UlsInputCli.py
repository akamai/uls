        ...
        while self.running is False and self.rerun_counter <= self.rerun_retries and not self.run_once:
            ...
            # ETP config
                ...
                if not self.rawcmd:
                    ...
                    my_feed = self._feed_selector(self.feed, product_feeds)
                    if my_feed == "DNS":
                        aka_log.log.warning(f"SIA-DNS FEED Improvement: Akamai SIA team has provided a smoother way of delivering huge log streams such as the DNS one to SIEM. Please have a look here: <2do>")
                        time.sleep(3)
